<template>
  <div>
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-model-item
                ...
                prop="icon"
              >
                <div v-if="form.icon" @click="selectIcon" class="aithings-microcheiria">
            ...
            <a-col :span="12">
              ...
              >
                <a-radio-group v-model="form.menuType" :disabled="formDisabled">
                  <a-radio :value="v.key" v-for="v in $DictList('menu_type')" :key="v.key">
                    {{ v.label }}
                  </a-radio>
                </a-radio-group>
              </a-form-model-item>
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-model-item
                ...
                prop="weigh"
              >
                <a-input-number v-model="form.weigh" :min="1" :max="999" :precision="0" style="width: 100%" placeholder="请输入排序, 1~999"/>
              </a-form-model-item>
            </a-col>
          </a-row>
          <!-- 是否外链 显示状态 -->
            ...
            <a-col :span="12">
              ...
              >
                <a-radio-group v-model="form.isFrame">
                  <a-radio :value="v.key" v-for="v in $DictList('yes_no')" :key="v.key">
                    {{ v.label }}
                  </a-radio>
                </a-radio-group>
              </a-form-model-item>
            ...
            <a-col :span="12">
              ...
              >
                <a-radio-group v-model="form.visible">
                  <a-radio :value="v.key" v-for="v in $DictList('display_status')" :key="v.key">
                    {{ v.label }}
                  </a-radio>
                </a-radio-group>
              </a-form-model-item>
          ...
          <a-row v-if="form.isFrame != 1">
            ...
            <a-col :span="12">
              <a-form-model-item
                ...
                prop="jumpPath"
              >
                <a-input v-model="form.jumpPath" placeholder="请输入重定向地址"/>
              </a-form-model-item>
            </a-col>
          </a-row>
          <!-- 是否隐藏子菜单 是否缓存 -->
    ...
    <!-- 图标相关 -->
    <a-modal :title="this.$t('public.select_icon')" v-model="iconVisible" width="800px">
      <icon-selector
        zIndex="2000"
        width="100%"
        v-model="form.icon"
        :loadType="['brand_logo', 'application', 'editor', 'data', 'iconfont']"
...
import { addDisparkMenu, editDisparkMenu, getDisparkMenuInfo} from "@/api/authority";
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      rules: {
        menuType: CommonRules(
          this.$t("authorityAdm.munu.inputInfo1"),
          "number"
        ),
        ...
        path: CommonRules(this.$t("authorityAdm.munu.inputInfo9")),
        component: CommonRules(this.$t("authorityAdm.munu.inputInfo11")),
  ...
  methods: {
    async init(arr, id, type) {
      ...
      if (type == "edit") {
        ...
        let res = await getDisparkMenuInfo({ menuId: id });
        if (res.code === 0) {
          let result = res.data;
    ...
    handleOk() {
      this.$refs["ruleForm"].validate(async (valid) => {
        ...
        let res;
        if (!this.form.menuId) {
          res = await addDisparkMenu(this.form);
        } else {
          res = await editDisparkMenu(this.form);
        }
        this.confirmLoading = false;
        this.toast(res);
        if (res.code === 0) {
