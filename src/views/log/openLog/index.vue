  ...
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="48">
          <a-col :md="4" :sm="24">
            <a-form-item>
              <a-input v-model="queryParam.queryObj.did" placeholder="请输入开放者账号" :allowClear="true"  @keyup.enter.native="query"/>
            </a-form-item>
          </a-col>
          <a-col :md="4" :sm="24">
            <a-form-item>
              <a-select v-model="queryParam.queryObj.isOnline" placeholder="请选择部署方式" :allowClear="true" @keyup.enter.native="query">
          ...
          <a-col :md="4" :sm="24">
            <a-form-item>
              <a-select v-model="queryParam.queryObj.isOnline" placeholder="请选择运行状态" :allowClear="true" @keyup.enter.native="query">
                <a-select-option v-for="v in $DictList('online_status')" :key="v.key" :value="v.key">
                  {{ v.label }}
                </a-select-option>
              </a-select>
    ...
    <a-table size="small" rowKey="id" :data-source="dataSource" :columns="columns" :loading="loading" :pagination="pagination" @change="onChangePagination">
      <template slot="action" slot-scope="text, record">
        <a-button type="link" size="small" icon="eye" @click="linkToPage('/log/open-log/details', record)">
          查看
        </a-button>
      </template>
    </a-table>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { dataIndex: "deviceName", title: "开发者账号", scopedSlots: { customRender: "deviceName" } },
        { dataIndex: "activeStatus", title: "部署方式", scopedSlots: { customRender: "activeStatus" } },
        { dataIndex: "onlineStatus", title: "最近访问", scopedSlots: { customRender: "onlineStatus" } },
        { dataIndex: "productName", title: "运行状态", scopedSlots: { customRender: "productName" } },
