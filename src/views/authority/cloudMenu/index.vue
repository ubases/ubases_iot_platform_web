            ...
            <a-col v-bind="aColFlex">
              <a-form-item>
                <a-input
                  ...
                  :placeholder="$t('authorityAdm.munu.inputName')"
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          ...
          <a-button
            ...
            size="small"
            icon="plus"
            :disabled="record.isFrame === 1"
            @click="handleNew(record)"
          >
...
<script>
import { getCloudMenuList, deleteCloudMenu } from "@/api/authority";
...
export default {
  ...
  methods: {
    queryList() {
      this.loading = true;
      getCloudMenuList(this.queryParam)
        .then((res) => {
          if (res.code == 0) {
    ...
    handleDel(id) {
      this.$Delete(this, "").then((res) => {
        deleteCloudMenu({ ids: [id] }).then((res) => {
          this.toast(res);
          this.queryList();
        });
      });
